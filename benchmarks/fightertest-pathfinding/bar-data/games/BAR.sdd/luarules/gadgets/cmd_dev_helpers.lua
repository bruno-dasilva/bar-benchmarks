...
if gadgetHandler:IsSyncedCode() then
	...
	function fightertest(_, line, words, playerID, action)
		...
		if fightertestactive then
			...
			-- benchmark-results.json: same schema as benchmark_playback.lua so the
			-- bar-benchmarks collector can treat fightertest and playback runs
			-- uniformly. Headless has no lobby menu, so the SendLuaMenuMsg path
			-- above is a no-op there; this file write is how results escape.
			local PCT_LIST = { 0, 1, 2, 5, 10, 20, 35, 50, 65, 80, 90, 95, 98, 99, 100 }
			local streams = {}
			for _, sk in ipairs({ "Sim", "Update", "Draw" }) do
				local ms = stats[sk]
				if ms and ms.count and ms.count > 0 then
					local pct = {}
					for _, p in ipairs(PCT_LIST) do
						pct[tostring(p)] = ms.percentiles[p] or 0
					end
					streams[sk:lower()] = {
						count       = ms.count,
						mean_ms     = ms.mean,
			...
			local wall_s = fightertestStartTimer
				and Spring.DiffTimers(Spring.GetTimer(), fightertestStartTimer) or 0
			local simFrames = #fighterteststats.simFrameTimes
			local report = {
				...
				wall_s          = wall_s,
				fps             = (wall_s > 0) and (simFrames / wall_s) or 0,
				streams         = streams,
				units_created   = fighterteststats.numunitscreated,
				units_destroyed = fighterteststats.numunitsdestroyed,
				command         = fighterteststats.fightertestcommand,
			}
			local path = "benchmark-results.json"
			local fout = io.open(path, "wb")
			if fout then
				fout:write(Json.encode(report))
				fout:write("\n")
				fout:close()
				Spring.Echo("[Fightertest] wrote results to " .. path)
			else
				Spring.Echo("[Fightertest] ERROR: cannot open " .. path .. " for writing")
			end
			...
			lastUpdateTimerUs = Spring.GetTimerMicros()
			fightertestStartTimer = Spring.GetTimer()
		end
